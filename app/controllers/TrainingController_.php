	...
	private function doAddEditView() {
		...
		* Participants */
		...
		if (! $this->setting ( 'display_middle_name' )) {
			...
			$personsFields = array ('person_id' => $this->tr ( 'ID         ' ),'first_name' => $this->tr ( 'First Name' ), 'last_name' => $this->tr ( 'Last Name' ) );
		} else if ($this->setting ( 'display_middle_name_last' )) {
			$personsFields = array ('person_id' => $this->tr ( 'ID         ' ),'first_name' => $this->tr ( 'First Name' ), 'last_name' => $this->tr ( 'Last Name' ), 'middle_name' => $this->tr ( 'Middle Name' ));
		} else {
			$personsFields = array ('person_id' => $this->tr ( 'ID         ' ),'first_name' => $this->tr ( 'First Name' ), 'middle_name' => $this->tr ( 'Middle Name' ), 'last_name' => $this->tr ( 'Last Name' ));
		}
		...
		if ( $filename ){
			...
			if(!trim($values['training_title_option_id'])){
				$status->addError ( 'title_option_id', t ( 'Your changes have been not saved: Type of training is required.' ) );
                        ...
                        if(!trim($values['training_level_id'])){
				$status->addError ( 'training_level_id', t ( 'Your changes have been not saved: Level of training is required. is here'.$rows[33][2] ) );
