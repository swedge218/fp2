			...
			*/
			...
			if ( $this->getSanParam ( 'training_location_id' ) == "" ){
				$status->addError( 'select_training_location', t('Training Location is required.') );
                        ...
                        if((!isset($this->getSanParam('start-day')) ) && isset($this->getSanParam('end-day'))){
                            $startDay = $this->getSanParam('end-day');
                        }else{
                           $startDay = "00"; 
                        }
                        
                        if((!isset($this->getSanParam('start-month')) ) && isset($this->getSanParam('end-month'))){
                            $startMonth = $this->getSanParam('end-month');
                        }else{
                           $startMonth = "00"; 
                        }
                        
                        if((!isset($this->getSanParam('start-year')) ) && isset($this->getSanParam('end-year')) ){
                            $startYear = $this->getSanParam('end-year');
                        }else{
                            $startYear = "0000";
                        }
                       ...
                       $startYear = "0000";
                       /*
			if ( trim(@$this->getSanParam('start-year')) == ""  || trim(@$this->getSanParam('start-month')) == "" || trim(@$this->getSanParam('start-day')) == "" ){
				//$status->addError( 'start-day', t('Start date is required.') );
                            $this->getSanParam('start-year') = $this->getSanParam('end-year');
                            $this->getSanParam('start-month') = $this->getSanParam('end-month');
                            $this->getSanParam('start-day')  = $this->getSanPram('end-day');
				...
				if ($training_end_date != '--') {
					if (strtotime ( $training_end_date ) < strtotime ( $training_start_date )) {
                        ...
                        $training_start_date = ($startDay) . '-' . ($startMonth) . '-' . ($startYear);
...
//error_reporting(E_ALL);
		...
		if ( $filename ){
			...
			$values['training_organizer_phrase'] = $rows[9][2];
                        ...
                        //echo 'End Date '.$rows[11][3].'<br/><br/>';
                        //echo 'Start Date '.$rows[10][3].'<br/><br/>';exit;
				 //$rows[11][3] = "22/07/2015";
                                //$rows[10][3] =  "02/07/2015";
                                 //exit;
			...
			if(!trim($rows[11][3])){
				...
				$true = $status->isValidDate ( $this, 'end-day', t ( 'Your changes have been not saved: Training end date'), $values['training_end_date'] );
			...
			if(!trim($values['training_title_option_id'])){
				$status->addError ( 'title_option_id', t ( 'Your changes have been not saved: Type of training is required.' ) );
                       ...
                       // foreach($rows as $row){
                        ...
                        for($v=31; $v<34; $v++){
                           // print_r($rows[$v][3]);echo '<br/><br/>';
                            if(!empty($rows[$v][3])){
                                $values['training_level_option_id'] = 0;
                                //echo $v;
				...
				$status->addError ( 'training_level_option_id', t ( 'Your changes have been not saved: Level of training is required. '.$rows[34][2] ) );
			
                                
                        }
                        
                        if(empty($rows[12][2])){
                                    $status->addError ( 'training_location_id', t ( 'Your changes have been not saved: Training Location is Required. '.$rows[12][2] ) );
                                }
                      // print_r($rows);
		...
		if($status->hasError() === 0){ //contunue parse persons if required training info is validated
			try{
				...
				$values['training_location_id'] =""; // trim($rows[12][2]); // by default 'unknown'
                                ...
                                //echo $stateLocationid;exit;
                                //var_dump($stateLocationid);echo 'heeelo';exit;
                                //echo $stateLocationid; echo 'hello hello';exit;
                                if(empty($stateLocationid)){
                                    $status->addError ( 'training_location_id', t ( 'Your changes have been not saved: Training Location is not valid. '.$rows[12][2] ) );
                                }
                                ...
                                //echo $values['training_location_id'];exit;
