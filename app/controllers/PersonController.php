		...
		*/
			...
			if ($loc){
				if ($criteria ['person_type'] == 'is_everyone' || $criteria['person_type']=='is_participant') {
					...
					$sql = '
					SELECT DISTINCT p.id, p.last_name, p.middle_name, p.first_name, p.gender, q.qualification_phrase,p.primary_qualification_option_id,p.phone_mobile,p.email, f.facility_name, province_name,province_id,district_name,district_id,region_c_name,region_c_id
				...
				if ($criteria ['person_type'] == 'is_everyone') {
					// left join instead of inner for everyone
					$sql = 'SELECT DISTINCT p.id, p.last_name, p.middle_name, p.first_name, p.gender,p.phone_mobile,p.email, q.qualification_phrase, f.facility_name,p.primary_qualification_option_id
				...
				$sql = '
                                        SELECT DISTINCT p.id, p.last_name, p.middle_name, p.first_name, p.gender, q.qualification_phrase,p.primary_qualification_option_id,p.phone_mobile,p.email, f.facility_name
					FROM person AS p,
					LEFT JOIN person_qualification_option AS q ON p.primary_qualification_option_id = q.id,
					LEFT JOIN (SELECT flv.facility_name as facility_name,flv.location_id, flv.id as facility_id,flv.lga_id as region_c_id,flv.lga as region_c_name,flv.state_id as district_id, flv.state as district_name, flv.geo_zone_id as province_id, flv.geo_zone as province_name FROM facility_location_view as flv
