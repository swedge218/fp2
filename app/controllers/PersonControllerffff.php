		...
		*/
			...
			if ($loc){
				if ($criteria ['person_type'] == 'is_everyone') {
					...
					$sql = '
					SELECT DISTINCT p.id, p.last_name, p.middle_name, p.first_name, p.gender,p.certification, q.qualification_phrase, f.facility_name, '.implode(',',$field_name).'
					FROM person as p
				...
				if ($criteria ['person_type'] == 'is_everyone') {
					// left join instead of inner for everyone
					$sql = '
					SELECT DISTINCT p.id, p.last_name, p.middle_name, p.first_name, p.gender,p.certification, q.qualification_phrase, f.facility_name
					FROM person as p
					LEFT JOIN person_qualification_option as q ON p.primary_qualification_option_id = q.id
					LEFT JOIN facility as f ON p.facility_id = f.id ';
...
/*
		...
		if( $this->getSanParam('outputType') == 'csv' )
             ...
             */
                $this->_redirect ( "../html/templates/Hr_upload_template.xlsx" ) ;
	}
}
