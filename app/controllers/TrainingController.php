		...
		if (! $this->hasACL ( 'training_organizer_option_all' )) {
			...
			foreach ( $training_organizer_array as $orgOption ) {
				...
				$allowIds [] = $orgOption ['id'];
                        ...
                        print "Iis not all acl that is being considered";
		}
			...
			if(!trim($values['training_title_option_id'])){
				$status->addError ( 'title_option_id', t ( 'Your changes have been not saved: Type of training is required.' ) );
                       ...
                       // foreach($rows as $row){
                        ...
                        $values_person['training_level_option_id'] = trim($values['training_level_option_id']);
                        //echo $values['training_level_option__id'];exit;
                        if(!$values['training_level_option_id']){
				...
				if ($training_id > 0) {
					...
					for($i=36; $i< sizeof($rows); $i++){
							...
							!empty($rows[$i][10]) || !empty($rows[$i][12])){
								...
								$pErrs = $importTrainingHelper->checkExcelRequiredFieldsPerson($rows,$errs,$i);
                                                               // print_r($pErrs);
								...
								if ( !$trainer_id ) { //add new person to Person
									...
									//optional fields
                                                                        ...
                                                                        list($values_person,$dataErrs,$to_fix,$mes_person,$mes_facility) = $importTrainingHelper->parsePersonDataToArray($values_person,$rows,$status,$i);
                                                                        //print_r($dataErrs);
